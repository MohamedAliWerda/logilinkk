...
export class MailService {
  ...
  async sendFeedbackReminder(to: string, companyName: string, postTitle: string, feedbackUrl: string): Promise<void> {
    if (!this.transporter) {
      throw new Error('SMTP transporter not configured');
    }

    const subject = 'Votre avis compte – Évaluez votre expérience avec un diplômé ISGIS';

    const html = `
      <div style="font-family: Arial, Helvetica, sans-serif; line-height: 1.6; color: #1f2937; max-width: 600px; margin: 0 auto;">
        <div style="background: #f97316; padding: 24px 32px; border-radius: 8px 8px 0 0;">
          <h1 style="color: #fff; margin: 0; font-size: 20px;">LogiLink – Formulaire de Feedback</h1>
        </div>
        <div style="background: #fff; padding: 32px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 8px 8px;">
          <p>Bonjour <strong>${this.escapeHtml(companyName)}</strong>,</p>
          <p>
            Il y a maintenant <strong>6 mois</strong> que vous avez publié l'offre d'emploi
            <em>« ${this.escapeHtml(postTitle)} »</em> sur <strong>LogiLink</strong>.
          </p>
          <p>
            Nous aimerions recueillir votre retour d'expérience sur le diplômé ISGIS que vous avez
            peut-être recruté. Votre avis nous aide à améliorer continuellement la qualité de
            nos formations et l'adéquation avec les besoins du marché.
          </p>
          <div style="background:#fef9f0;border-left:4px solid #f97316;padding:14px 18px;border-radius:4px;margin:20px 0;">
            <p style="margin:0;font-size:14px;">
              Le formulaire prend moins de <strong>3 minutes</strong> à remplir.
              Il vous sera d'abord demandé si vous avez effectivement recruté un diplômé ISGIS
          ...
          <p style="margin-top: 16px; font-size: 12px; color: #6b7280; text-align: center;">
            Si le bouton ne fonctionne pas, copiez ce lien dans votre navigateur :<br/>
            <a href="${feedbackUrl}" style="color:#f97316;">${feedbackUrl}</a>
          </p>
          <p style="margin-top: 32px; font-size: 13px; color: #6b7280;">
            Merci de votre confiance et de votre contribution.<br/>
    ...
    const text = `Bonjour ${companyName},\n\nIl y a maintenant 6 mois que vous avez publié l'offre "${postTitle}" sur LogiLink.\n\nNous aimerions recueillir votre retour d'expérience sur le diplômé ISGIS que vous avez peut-être recruté.\n\nAccédez au formulaire : ${feedbackUrl}\n\n— L'équipe LogiLink / ISGIS`;
