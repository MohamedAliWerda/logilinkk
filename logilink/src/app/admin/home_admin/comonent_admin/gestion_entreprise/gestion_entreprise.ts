import { Component, OnInit } from '@angular/core';  // ✅ ajouter OnInit ici
...
export class GestionEntrepriseComponent implements OnInit {
  ...
  get nbEnAttente() { return this.entreprisesEnAttente.length; }
  get nbValidees() { return this.entreprisesValidees.length; }
  get totalPostes() { return this.entreprisesValidees.reduce((s, e) => s + e.nbPostes, 0); }

  get filteredValidees() {
  ...
  getTotalEtudiants(entreprise: Entreprise): number {
    return entreprise.postes?.reduce((s, p) => s + p.nbEtudiants, 0) ?? 0;
  }

  ouvrirPostes(entreprise: Entreprise) {
  ...
  confirmerSuppAcces(entreprise: Entreprise) {
  ...
  supprimerAcces() {
    if (this.entrepriseToSupp) {
      this.entreprisesValidees = this.entreprisesValidees.filter(e => e.id !== this.entrepriseToSupp!.id);
  ...
  validerEntreprise(entreprise: Entreprise) {
  ...
  rejeterEntreprise(entreprise: Entreprise) {
    this.entreprisesEnAttente = this.entreprisesEnAttente.filter(e => e.id !== entreprise.id);
  }
