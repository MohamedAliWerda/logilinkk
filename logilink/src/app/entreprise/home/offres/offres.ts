...
export class Offres implements OnInit {
  ...
  constructor(
    ...
    this.offreForm = this.fb.group({
      ...
      source: ['partenaire', Validators.required],
      salaireMin: [null],
      salaireMax: [null],
  ...
  get totalCompetences(): number {
    ...
    this.offres.forEach(offre => offre.competences.forEach((comp: string) => allCompetences.add(comp)));
  ...
  filterOffres() {
    if (!this.searchQuery.trim()) {
      ...
      this.filteredOffres = this.offres.filter(offre =>
        ...
        offre.competences.some((c: string) => c.toLowerCase().includes(query))
      );
    }
  }
  ...
  openCreateModal() {
    ...
    this.offreForm.reset({ typeContrat: 'CDI', source: 'partenaire' });
    this.tempSkills = [];
  }

  openEditModal(offre: Offre) {
    ...
    this.offreForm.patchValue({
      ...
      source: offre.source || 'partenaire',
      salaireMin: offre.salaireMin,
      salaireMax: offre.salaireMax,
      description: offre.description
    });
    this.tempSkills = [...offre.competences];
  }
  ...
  addSkill() {
    if (this.newSkill.trim() && !this.tempSkills.includes(this.newSkill.trim())) {
