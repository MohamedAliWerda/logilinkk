...
export class SupabaseService {
  ...
  async sendSocieteApprovalEmail(email: string, companyName: string): Promise<void> {
    const apiUrl = `${environment.apiUrl}/admin/societe/send-approval-email`;
    await firstValueFrom(this.http.post<any>(apiUrl, { email, companyName }));
  }

  async sendSocieteRejectionEmail(email: string, companyName: string): Promise<void> {
    const apiUrl = `${environment.apiUrl}/admin/societe/send-rejection-email`;
    await firstValueFrom(this.http.post<any>(apiUrl, { email, companyName }));
  }

  async sendSocietePendingEmail(email: string, companyName: string): Promise<void> {
