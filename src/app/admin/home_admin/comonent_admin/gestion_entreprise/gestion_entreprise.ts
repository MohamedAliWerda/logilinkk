...
export class GestionEntrepriseComponent implements OnInit, OnDestroy {
  ...
  async validerEntreprise(entreprise: Entreprise): Promise<void> {
    try {
      ...
      const email = updated?.email ?? updated?.Email ?? '';
      const companyName = updated?.denomination_sociale ?? updated?.nom ?? entreprise.nom ?? '';
      if (email) {
        this.supabase.sendSocieteApprovalEmail(email, companyName).catch((err: unknown) => {
  ...
  async rejeterEntreprise(entreprise: Entreprise): Promise<void> {
    try {
      ...
      const companyName = updated?.denomination_sociale ?? updated?.nom ?? entreprise.nom ?? '';
      if (email) {
        this.supabase.sendSocieteRejectionEmail(email, companyName).catch((err: unknown) => {
